...
- `crates/ticket_db/` - Postgres ticket database (tickets, edges, activities, metadata; `ur_tickets` DB)
- `crates/workflow_db/` - Postgres workflow database (workflow state, events, intents, comments, worker/slot lifecycle; `ur_workflow` DB)
...
- **Single config file**: All user configuration lives in `ur.toml` (`$UR_CONFIG/ur.toml`, default `~/.ur/ur.toml`). Do NOT create separate config files for new features — extend `ur.toml` instead. Hostexec Lua transforms are the exception: `$UR_CONFIG/hostexec/<command>.lua` is discovered by filename, while `projects.<key>.hostexec` in `ur.toml` grants project access.
