...
- Main gRPC server runs on `127.0.0.1:$daemon_port` (TCP, default 42069); per-agent gRPC servers bind to `0.0.0.0` on an OS-assigned port, reachable from containers via Docker internal DNS (`network.urd_hostname`)
