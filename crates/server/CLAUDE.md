...
- Two gRPC servers: host server on `127.0.0.1:$server_port` (Core + RAG + Ticket, no auth) and worker server on `0.0.0.0:$worker_port` (HostExec + RAG + Ticket, auth interceptor validates worker-id/secret via WorkerManager)
