...
impl RunOptsBuilder {
    ...
    /// Add project brain directory mount.
    ///
    /// - If `brain_dir` is `None`, this is a no-op.
    /// - Otherwise, template-resolves via [`resolve_template_path`]. The result must be a
    ///   [`ResolvedTemplatePath::HostPath`]; a `ProjectRelative` result returns `Err` because
    ...
    pub fn add_brain_dir(
        mut self,
        brain_dir: &Option<String>,
        host_config_dir: &Path,
    ) -> Result<Self, String> {
        let Some(template) = brain_dir.as_deref() else {
            return Ok(self);
        };

        let resolved = resolve_template_path(template, host_config_dir)
            .map_err(|e| format!("failed to resolve brain_dir: {e}"))?;

        let host_path = match resolved {
            ResolvedTemplatePath::HostPath(host_path) => host_path,
            ResolvedTemplatePath::ProjectRelative(_) => {
                return Err("brain_dir resolved to a project-relative path, \
                     which is not supported (use an absolute path or %URCONFIG%)"
                    .to_string());
            }
        };
...
mod tests {
    ...
    #[test]
    fn add_brain_dir_none_is_noop() {
        let tmp = tempfile::tempdir().unwrap();
        let req = RunOptsBuilder::new("img".into(), "name".into(), "net".into())
            .add_brain_dir(&None, tmp.path())
            .unwrap()
            .build();
    ...
    /// Test the HostPath success path: directory is created if missing, chowned,
    /// and a single read-write volume mount to `/brain` is pushed.
    ///
    /// Restricted to Linux because `chown` to `WORKER_UID` (1000) requires either
    /// running as root or being that user; on macOS the test runner is typically a
    /// different uid and the syscall returns EPERM.
    #[cfg(target_os = "linux")]
    #[test]
    fn add_brain_dir_host_path_creates_dir_and_mounts() {
        let tmp = tempfile::tempdir().unwrap();
        let host_config_dir = tmp.path();
        // Use an absolute path for the brain dir (no pre-existing directory).
        let brain_host_path = tmp.path().join("shared_brain");
        let brain_dir = Some(brain_host_path.display().to_string());
        ...
        // Directory should have been created.
        assert!(
            brain_host_path.exists(),
            "brain dir should be created on host"
        );
        ...
        assert_eq!(
            req.volumes[0].host_path,
            brain_host_path.display().to_string()
        );
    ...
    #[test]
    fn add_brain_dir_project_relative_returns_error() {
        let tmp = tempfile::tempdir().unwrap();
        // %PROJECT%/... resolves to ProjectRelative — should be rejected defensively.
        let brain_dir = Some("%PROJECT%/brain".to_string());

        let result = RunOptsBuilder::new("img".into(), "name".into(), "net".into())
            .add_brain_dir(&brain_dir, tmp.path());
