...
- `worker launch` assumes `ur-worker-rust:latest` image exists, then calls ProcessLaunch RPC; `-f` stops existing process first via ProcessStop RPC
- `worker stop` / `worker kill` both call ProcessStop RPC
