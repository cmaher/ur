...
- `worker launch` resolves the image from the project's `container.image` config (falls back to `ur-worker-rust:latest`), then calls WorkerLaunch RPC; `-f` stops existing worker first via WorkerStop RPC
