...
/// Entry point for the ur CLI. Accepts a `CliRegistry` so that plugin binaries
/// (e.g. ur-pro) can inject plugins before running the standard CLI logic.
///
/// With an empty registry, this behaves identically to the stock `ur` binary.
pub fn run(mut registry: CliRegistry) -> Result<()> {
    let output_format = output::resolve_output_format_early();

    // Build the clap command, allowing plugins to register subcommands.
    let base_cmd = <Cli as clap::CommandFactory>::command();
    let augmented_cmd = registry.apply_register_cli(base_cmd);

    let matches = match augmented_cmd.try_get_matches() {
        Ok(m) => m,
        Err(e) => {
    ...
    // Check if a plugin handles this invocation before the normal dispatch.
    if registry.apply_handle_cli(&matches)? {
        return Ok(());
    }
    ...
    if cli.describe {
        ...
        let augmented = registry.apply_register_cli(cmd);
        let schema = describe::describe_command(&augmented);
    ...
    if let Err(err) = rt.block_on(run_async(cli, &mut registry, &out)) {
...
fn resolve_workspace_dir(workspace: Option<PathBuf>) -> Result<String> {
    match workspace {
        Some(path) => {
...
async fn run_async(cli: Cli, registry: &mut CliRegistry, output: &OutputManager) -> Result<()> {
    ...
    // Configure plugins with their TOML tables from ur.toml.
    ...
    let port = config.server_port;
    let compose = compose::compose_manager_from_config(&config, registry)?;
