//! CozoDB Backup API Evaluation and Persistence Strategy
//!
//! # Findings
//!
//! ## SQLite Backend
//! CozoDB supports a SQLite storage backend (`DbInstance::new("sqlite", path, "")`) that
//! persists all data to a single file on disk. The database file is created automatically
//! on first use and can be reopened across process restarts.
//!
//! ## Backup API (`db.backup_db()`)
//! - **Mechanism**: Opens a transaction on the source DB, performs a full key-range scan,
//!   and batch-writes all data into a new SQLite file at the target path. This is a
//!   logical copy at the storage engine level, not a filesystem copy.
//! - **Output format**: The backup is always an SQLite file, regardless of the source
//!   backend. This means backups from an in-memory DB are also SQLite files.
//! - **Safety during writes**: The backup opens a read transaction, so it captures a
//!   consistent snapshot. Concurrent writes are not blocked; they simply will not appear
//!   in the backup if they commit after the backup's transaction begins. This makes
//!   backup safe to run at any time without quiescing writes.
//! - **Target must be empty**: `backup_db()` fails if the target path already contains
//!   data. To overwrite a backup, delete the old file first.
//! - **Restore**: `restore_backup()` only works on an empty database (store_id == 0).
//!   It is intended for disaster recovery into a fresh instance, not incremental merges.
//!
//! ## Recommended Backup Approach
//!
//! **Interval**: Backup every 5 minutes while the server is running, configurable in
//! `ur.toml` under `[database]`:
//! ```toml
//! [database]
//! path = "~/.ur/db/tickets.db"
//! backup_path = "~/.ur/backups/tickets.db"  # or a Google Drive sync folder
//! backup_interval_secs = 300
//! ```
//!
//! **Destination**: The backup path should be a single file path. The backup manager
//! should delete the old backup file before writing a new one (since `backup_db()`
//! requires an empty target). Alternatively, rotate backups with timestamped filenames.
//!
//! **Implementation pattern**:
//! 1. Spawn a tokio task that ticks on the configured interval
//! 2. On each tick: remove the backup file if it exists, then call `db.backup_db(path)`
//! 3. Log success/failure but do not crash on backup failure
//!
//! ## Gotchas
//! - **File locking**: CozoDB/SQLite holds a write lock on the primary DB file. Only one
//!   process should open a given DB path at a time. The backup file, however, is only
//!   written during the backup call and can be freely copied afterward.
//! - **Backup size**: The backup is a full logical copy, not incremental. For small
//!   ticket databases (thousands of tickets), this is negligible (< 10 MB). For very
//!   large datasets, consider the I/O cost of full backups at high frequency.
//! - **No incremental backup**: CozoDB does not support incremental or WAL-based backup.
//!   Each backup is a full snapshot.
//! - **Corruption risk**: The primary DB uses SQLite's ACID guarantees. As long as the
//!   process is not killed during a write (or the disk doesn't fail), corruption is
//!   unlikely. The backup provides a second copy for belt-and-suspenders safety.
//! - **Restore erases everything**: `restore_backup()` only works on a fresh/empty DB.
//!   To restore, you must create a new `DbInstance` pointing at a fresh path, restore
//!   into it, then swap it in.
