...
Two modes:
- `ur-workerd init` — synchronous initialization: skills, git hooks, hostexec shims. Run by entrypoint before anything else.
- `ur-workerd` (no args) — background daemon loop. Stays alive for future daemon uses.
...
- Generates shims in `~/.local/bin/` that call `ur-tools host-exec <command> "$@"`
