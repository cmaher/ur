...
End-to-end acceptance tests that exercise the full stack: urd daemon, container lifecycle, and worker command binaries.
...
- Tests use pre-built binaries from `target/` (urd, ur) and worker commands (ur-ping, git) baked into the container image
