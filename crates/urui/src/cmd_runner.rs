...
///
/// Sets the `autoapprove` metadata key on the ticket, which the github poller
/// picks up to advance the workflow past the review gate.
async fn approve_workflow(port: u16, ticket_id: &str) -> Result<(), String> {
    use ur_rpc::connection::connect;
    use ur_rpc::proto::ticket::SetMetaRequest;
    use ur_rpc::proto::ticket::ticket_service_client::TicketServiceClient;
    use ur_rpc::ticket_meta::AUTOAPPROVE;
    ...
    client
        .set_meta(SetMetaRequest {
            ticket_id: ticket_id.to_owned(),
            key: AUTOAPPROVE.to_owned(),
            value: "true".to_owned(),
        })
        .await
        .map_err(|e| {
            error!(port, %ticket_id, error = %e, "v2: workflow approve failed");
            e.to_string()
        })?;
    Ok(())
}
