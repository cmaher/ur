...
- `docs/codeflows/process-launch-credentials.md` — Process launch and credential injection (how containers get agent credentials — Claude Keychain/file, Codex host `auth.json` — and why each is mounted as a single file, never a whole `~/.claude`/`~/.codex` directory)
