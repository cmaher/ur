...
- `docs/codeflows/process-launch-credentials.md` — Process launch and credential injection (Claude/AGY OS keyrings and file fallbacks, Codex host `auth.json`, AGY interactive bootstrap, and why each credential is mounted as one file rather than a whole agent state directory)
