...
This creates two logical databases before `ur-server` runs its migrations:
...
the two above. The DATABASE_URL passed to `ur-server` continues to target the user-configured
`db.name` (default `ur`) which may be updated in a future migration ticket.
