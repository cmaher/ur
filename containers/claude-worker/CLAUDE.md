...
- Worker command binaries (`ur-ping`, `workertools`, `workerd`, `tk`, `ur-osc8`) are cross-compiled and staged into `bin/` by `stage-workercmd.sh`, then copied into the image at `/usr/local/bin/`
