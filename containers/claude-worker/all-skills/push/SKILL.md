...
4. **Pull Request**: Run `gh pr view --json url,state,number` to check if a PR already exists for this branch.
...
5. **Ticket metadata**: If a ticket ID is available (from `$ARGUMENTS` or context):
   - On **first push** (no upstream tracking existed before step 3), set the branch: `ur ticket set-meta <id> branch <branch-name> --output json`
...
6. **Merge conflicts**: If the push or PR creation fails due to merge conflicts, attempt to resolve them:
...
7. **CI failures**: After pushing, check CI status with `gh pr checks`.
