...
description: Use when implementing an epic with subagent dispatch — works directly for a single ticket, dispatches subagents in parallel (or sequentially) for epics (tickets with open descendants)
---

# Implement Tickets (with Subagent Dispatch)

Implement one or more tickets. An **epic** is any ticket with open descendants — detected at runtime, not by ticket type. For epics, this skill **dispatches subagents** to do the work.

> If you want everything done in a single context without subagents, use the **implement** skill instead.
...
No subagents needed for a single ticket. Just do the work.
